...
class LasingOff(object):
    ...
    def Generate(self, savetofile=False):
        ...
        sio.savemat('lasing_off.mat', {'data':data})
    ...
    def processEvent(self, evt):
        ...
        data["xprofile"] = image_stats[0].xProfile
        data["yprofile"] = image_stats[0].yProfile
        data["yCOMslice"] = image_stats[0].yCOMslice
        data["yRMSslice"] = image_stats[0].yRMSslice
        data["ebeamcharge"] = shot_to_shot.ebeamcharge
        data["img"] = img
        data["dumpe"] = self.global_calibration.dumpe
        data.update(dict(vars(physical_units)))
        data.pop('valid')
